...
# Layer definitions.
concat = tf.compat.v1.keras.backend.concatenate
stack = tf.compat.v1.keras.backend.stack
K = tf.compat.v1.keras.backend
Add = tf.compat.v1.keras.layers.Add
LayerNorm = tf.compat.v1.keras.layers.LayerNormalization
Dense = tf.compat.v1.keras.layers.Dense
Multiply = tf.compat.v1.keras.layers.Multiply
Dropout = tf.compat.v1.keras.layers.Dropout
Activation = tf.compat.v1.keras.layers.Activation
...
def linear_layer(size,
    ...
    """
    linear = tf.compat.v1.keras.layers.Dense(size, activation=activation, use_bias=use_bias)
    if use_time_distributed:
        linear = tf.compat.v1.keras.layers.TimeDistributed(linear)
    return linear
...
def apply_mlp(inputs,
    ...
    """
    if use_time_distributed:
        hidden = tf.compat.v1.keras.layers.TimeDistributed(
            tf.compat.v1.keras.layers.Dense(hidden_size, activation=hidden_activation))(
            inputs)
        return tf.compat.v1.keras.layers.TimeDistributed(
            tf.compat.v1.keras.layers.Dense(output_size, activation=output_activation))(
            hidden)
    else:
        hidden = tf.compat.v1.keras.layers.Dense(
...
def apply_gating_layer(x,
    ...
    if dropout_rate is not None:
        x = tf.compat.v1.keras.layers.Dropout(dropout_rate)(x)

    if use_time_distributed:
        activation_layer = tf.compat.v1.keras.layers.TimeDistributed(
            tf.compat.v1.keras.layers.Dense(hidden_layer_size, activation=activation))(
            x)
        gated_layer = tf.compat.v1.keras.layers.TimeDistributed(
            tf.compat.v1.keras.layers.Dense(hidden_layer_size, activation='sigmoid'))(
            x)
    else:
        activation_layer = tf.compat.v1.keras.layers.Dense(
            hidden_layer_size, activation=activation)(
            x)
        gated_layer = tf.compat.v1.keras.layers.Dense(
            hidden_layer_size, activation='sigmoid')(
            x)

    return tf.compat.v1.keras.layers.Multiply()([activation_layer,
...
def gated_residual_network(x,
    ...
    if additional_context is not None:
        hidden = hidden + linear_layer(
            ...
            use_bias=False)(
            additional_context)
    hidden = tf.compat.v1.keras.layers.Activation('elu')(hidden)
    hidden = linear_layer(
        hidden_layer_size,
        activation=None,
        use_time_distributed=use_time_distributed)(
...
class TemporalFusionTransformer(object):
    ...
    def get_tft_embeddings(self, all_inputs):
        ...
        for i in range(num_categorical_variables):
            embedding = tf.compat.v1.keras.Sequential([
                tf.compat.v1.keras.layers.InputLayer([time_steps]),
        ...
        if self._static_input_loc:
            static_inputs = [tf.compat.v1.keras.layers.Dense(self.hidden_layer_size)(
                regular_inputs[:, 0, i:i + 1]) for i in range(num_regular_variables)
                                if i in self._static_input_loc] \
                            + [embedded_inputs[i][:, 0, :]
                               for i in range(num_categorical_variables)
                               if i + num_regular_variables in self._static_input_loc]
            static_inputs = tf.compat.v1.keras.backend.stack(static_inputs, axis=1)
        ...
        def convert_real_to_embedding(x):
            ...
            return tf.compat.v1.keras.layers.TimeDistributed(
                tf.compat.v1.keras.layers.Dense(self.hidden_layer_size))(
                x)

        # Targets
        obs_inputs = tf.compat.v1.keras.backend.stack([
        ...
        if unknown_inputs + wired_embeddings:
            unknown_inputs = tf.compat.v1.keras.backend.stack(
        ...
        known_combined_layer = tf.compat.v1.keras.backend.stack(
    ...
    def _build_base_graph(self):
        ...
        # Isolate Ocean-atmosphere inputs.
        other_inputs = known_combined_layer[:, :encoder_steps, :]
        ...
        def static_combine_and_mask(embedding):
            ...
            flatten = tf.compat.v1.keras.layers.Flatten()(embedding)
            ...
            sparse_weights = tf.compat.v1.keras.layers.Activation('softmax')(mlp_outputs)
            ...
            combined = tf.compat.v1.keras.layers.Multiply()(
        ...
        historical_features, historical_flags, _ = lstm_combine_and_mask(
            historical_inputs)
        other_features, other_flags, _ = lstm_combine_and_mask(other_inputs)
        ...
        other_lstm = get_lstm(return_state=False)(
            other_features, initial_state=[state_h, state_c])

        lstm_layer = concat([history_lstm, other_lstm], axis=1)

        # Apply gated skip connection
        input_embeddings = concat([historical_features, other_features], axis=1)
        ...
        attention_components = {
            ...
            # Variable selection weights of Ocean-atmosphere inputs
            'other_flags': other_flags[Ellipsis, 0, :]
        }
    ...
    def build_model(self):
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            outputs = tf.compat.v1.keras.layers.TimeDistributed(
                tf.compat.v1.keras.layers.Dense(self.output_size * len(self.quantiles))) \
            ...
            model = tf.compat.v1.keras.Model(inputs=all_inputs, outputs=outputs)
